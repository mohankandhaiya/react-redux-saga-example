...
import {fetchCategory, loginRequest, showTopics, deleteTopics, signupRequest, addTopics, editTopics, averageRating, totalRating, requestComments} from "../API";
...
function* DeleteTopicsSaga(action) {
    const deletedata = yield call (()=>deleteTopics(action.payload));
    console.log(deletedata);
    const data = yield call (()=>showTopics(action.payload.category));
    console.log(data);
    yield put({type: actions.TOPICS_SUCCESS, data: data });
}
...
function* UserIdSaga(action) {
    yield put({ type: actions.USER_ID_SUCCESS, data: action});
}
...
function* CommentSaga(action) {
    console.log(action);
    const data = yield call (()=>requestComments(action));
    console.log(data);
